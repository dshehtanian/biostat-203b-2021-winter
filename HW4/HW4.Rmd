...
```{r}
d30_true <- icu_analysis[icu_analysis$died_within_30d==1,1:ncol(icu_analysis)]
d30_false <- icu_analysis[icu_analysis$died_within_30d==0,1:ncol(icu_analysis)]
...
y_train <- ifelse(icu_train$died_within_30d == 1, 1, 0)
...
y_test <- ifelse(icu_test$died_within_30d == 1, 1, 0)
```
...
lgstc_reg <- glm(died_within_30d ~ gender + age_at_adm + marital_status + 
                   ...
                   hematocrit + wbc, data=icu_train, family = binomial)
summary(lgstc_reg)

#Logistic Regression with Lasso
lfit=glmnet(x_train, y_train, family = "binomial", alpha = 1)
summary(lfit)
...
```{r}
#Logistic Regression with Lasso
pred_val <- predict(lfit, newx = x_test, type = "response", s = lfit$lambda)
dev_val <- -2*colSums(y_test * log(pred_val) + (1 - y_test) * log(1 - pred_val))
tibble(lambda = lfit$lambda, dev_val = dev_val) %>% ggplot() + 
  geom_point(mapping = aes(x = lambda, y = dev_val)) + scale_x_log10() + 
