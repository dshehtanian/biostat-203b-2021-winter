...
x <- (model.matrix(died_within_30d ~ -1 + gender + age_at_adm + marital_status + 
                   ethnicity + heart_rate + 
                   non_invasive_blood_pressure_systolic + 
                   non_invasive_blood_pressure_mean + respiratory_rate + 
                   temperature_fahrenheit + bicarbonate + calcium + chloride +
                   creatinine + glucose + magnesium + potassium + sodium +
                   hematocrit + wbc, data=icu_imputed$Dataset_1) + 
      model.matrix(died_within_30d ~ -1 + gender + age_at_adm + marital_status + 
                   ethnicity + heart_rate + 
                   non_invasive_blood_pressure_systolic + 
                   non_invasive_blood_pressure_mean + respiratory_rate + 
                   temperature_fahrenheit + bicarbonate + calcium + chloride +
                   creatinine + glucose + magnesium + potassium + sodium +
                   hematocrit + wbc, data=icu_imputed$Dataset_3) +
      model.matrix(died_within_30d ~ -1 + gender + age_at_adm + marital_status + 
                   ethnicity + heart_rate + 
                   non_invasive_blood_pressure_systolic + 
                   non_invasive_blood_pressure_mean + respiratory_rate + 
                   temperature_fahrenheit + bicarbonate + calcium + chloride +
                   creatinine + glucose + magnesium + potassium + sodium +
                   hematocrit + wbc, data=icu_imputed$Dataset_3))/3
#None of the values for died within 30 days were imputed, thus the values in all three sets provided by miceRanger are the same
y <- ifelse(icu_imputed$Dataset_1$died_within_30d == "TRUE", 1, 0)
```
...
```{r}
d30_true <- icu_imputed$Dataset_1[died_within_30d == TRUE]
d30_false <- icu_imputed$Dataset_1[died_within_30d == FALSE]
...
train_data <- rbind(d30_true[smpl_true], d30_false[smpl_false])
test_data <- rbind(d30_true[-smpl_true], d30_false[-smpl_false])
```
2. Train the models using the training set.
```{r}
#Logistic Regression
lgstc_reg <- glm(died_within_30d ~ gender + age_at_adm + marital_status + 
                   ethnicity + heart_rate + 
                   non_invasive_blood_pressure_systolic + 
                   non_invasive_blood_pressure_mean + respiratory_rate + 
                   temperature_fahrenheit + bicarbonate + calcium + chloride +
                   creatinine + glucose + magnesium + potassium + sodium +
                   hematocrit + wbc, data=train_data, family = binomial)
summary(lgstc_reg)

#Logistic Regression with Lasso
#need to divide into train and test data
lfit=glmnet(x, y, family = "binomial", alpha = 1)
summary(lfit)
