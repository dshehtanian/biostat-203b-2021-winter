...
Removing the fread data from memory, as it is now a duplicate.
```{r}
rm(chartfilter)
gc()
```
...
Now that we have generated a smaller data set from this, we deallocate it from memory so we improve performance and don't cause issues due to lack of memory.
```{r}
rm(chartfilter_tble)
gc()
```
