...
```
```{r}
system.time(labevent <- fread(str_c(mimic_path, "/hosp/labevents.csv.gz"), 
                              select = c("subject_id", "hadm_id", "itemid",
                                         "charttime", "valuenum"), nThread = 8))
```

```{r}
semi_join(icu_table, as.tibble(labevent), by = c("subject_id", "hadm_id")) %>%
  fwrite("labevents_icustays.csv.gz", nThread = 8)
```

```{r}
system.time(labevents_tble <- fread("labevents_icustays.csv.gz", nThread = 6))
```
