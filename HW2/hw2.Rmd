...
```{r, eval = F}
chartfil_tble <- filter(chart_icu, itemid=c(220179, 220181, 220045, 220210, 
                                    223761, 220045)) %>% print(width = Inf)
```
...
chartfilter <- fread(str_c(mimic_path, 
  ...
  col.names = c("subject_id", "hadm_id","stayid","charttime","itemid","valuenum"
                )) %>% print(width = Inf)
```
...
```{r}
icutble_uniq <- icu_table %>% group_by(subject_id) %>% filter(rank(intime) == 1) %>%
  ungroup() %>% left_join(admit_table, by = c("subject_id", "hadm_id")) %>% 
...
```{r}
labmerge %>% right_join(icutble_uniq, by = c("subject_id", "hadm_id")) %>%
