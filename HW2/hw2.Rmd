...
Deaths in Each Year
```{r}
ggplot(admit_table,aes(hour(admittime))) + geom_bar() 
```
## Q5. `patient` data
...
There is a large amount of patients with an anchor_age of zero, this doesn't make sense, so this probably means those anchor_ages are missing, but we should verify. Otherwise, there appears to be a sinusoidal pattern, but overall decreasing pattern to the data.
