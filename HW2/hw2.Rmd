...
There is a large speed difference between read.csv (45.79s) and read_csv (6.29s). There is a smaller speed difference between read_csv and fread (1.88s). fread is the fastest, followed by read_csv and finally read.csv.
...
Death (use the hospital_expire_flag variable)
```{r}
admit_table %>% filter(hospital_expire_flag==0 & deathtime!=NA)
```
...
```
```{r}
#labevent <- fread('/usr/203b-data/mimic-iv/hosp/labevents.csv.gz', header = TRUE)
```
