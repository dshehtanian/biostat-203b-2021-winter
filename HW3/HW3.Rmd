...
**Solution:** Acknowledged and retained for reference.
...
**Note:** If you run this code on your personal computer you may need to change the username/password information to connect to your database.
...
labevents_tble <- tbl(con, Id(schema = "public", 
                      table = "mimic_labevents_icu")) %>% print(width = Inf)
```

7. Connect to `mimic_chartevents_icu` table.
```{r}
chartevents_tble <- tbl(con, Id(schema = "public", 
                        table = "mimic_chartevents_icu")) %>% print(width = Inf)
```
...
```{r}
icustays_tble %>% group_by(subject_id) %>% filter(rank(intime) == 1) %>% 
  ungroup() %>%  left_join(admissions_tble, by = c("subject_id", "hadm_id")) %>% 
  left_join(patients_tble, by = "subject_id") %>% mutate(age_at_adm = 
  ...
  show_query() %>% collect() %>% mutate(days_to_death = 
...
**Solution:**
See app.R file in the mimiciv_shiny directory.
