...
```{bash}
  ...
  echo ''
  zcat /usr/203b-data/mimic-iv/core/admissions.csv.gz \
  | awk -F, '{print $10}' | tail -n+2 | sort -u
```
...
```{bash}
  ...
  echo ''
  zcat /usr/203b-data/mimic-iv/core/admissions.csv.gz \
  | awk -F, '{print $11}' | tail -n+2 | sort -u
```
...
```{bash}
  ...
  echo ''
  zcat /usr/203b-data/mimic-iv/core/admissions.csv.gz \
  | awk -F, '{print $12}' | tail -n+2 | sort -u
```
